...
def run_gpu():
    print("--- 1. Writing remote code ---")
    with open("gemm_tiled.cu", "w") as f:
        f.write(cuda_source_code)
        
    print("--- 2. Compiling (nvcc -O3) ---")
    subprocess.run(["nvcc", "-O3", "-o", "gemm_tiled", "gemm_tiled.cu"], check=True)
    
    print("--- 3. Running on H100 GPU ---")
